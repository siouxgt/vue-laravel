...
class ReporteAdmin extends Model
{
    use HasFactory;
    ...
    public static function reportesAll(){
        return DB::select("SELECT r.id, r.nombre_reporte, to_char(r.created_at,'DD/MM/YYYY') AS fecha FROM reporte_admins AS r");
    }
   ...
   public static function directorioURG($parametros){
        $sql = "SELECT ur.id, ur.ccg, ur.nombre, (SELECT u.email FROM users AS u WHERE u.rol_id = 4 AND u.urg_id = ur.id LIMIT 1) AS email, (SELECT u.telefono FROM users AS u WHERE u.rol_id = 4 AND u.urg_id = ur.id LIMIT 1) AS telefono, TO_CHAR(ur.created_at,'DD/MM/YYYY') AS fecha_creacion
            FROM urgs AS ur";
        ...
        if($parametros->urg != ""){
            $sql .= " ".$condicion[$contador]." ur.id = ".$parametros->urg;
        }
        if($parametros->anio != null && $parametros->trimestre == null){
            $sql .= " ".$condicion[$contador]." extract(year from ur.created_at) = ".$parametros['anio'];
            $contador = 1;
            if($parametros->mismo_anio){
                $sql .= " ".$condicion[$contador]." ur.created_at <= '".$parametros->fecha_reporte."'";    
            }
        }
        if($parametros->de != null && $parametros->hasta != null){
            $sql .= " ".$condicion[$contador]." ur.created_at BETWEEN '".$parametros->de."' AND '".$parametros->hasta."'";
        }
        if($parametros->de == null && $parametros->hasta == null && $parametros->anio == null){
            $sql .= " ".$condicion[$contador]." ur.created_at <= '".$parametros->fecha_reporte."'";
        }
   ...
   public static function reporteInUrg($parametros){
        $sql = "SELECT i.id_incidencia, u.nombre, i.etapa, i.etapa_id, i.motivo, i.descripcion, u.ccg, to_char(i.created_at,'DD/MM/YYYY') AS fecha_creacion, i.sancion, CASE WHEN i.reporta = 1 THEN (SELECT CONCAT(u.nombre,' ',u.primer_apellido,' ',u.segundo_apellido) FROM users AS u WHERE u.id = i.user_creo) WHEN i.reporta = 3 THEN (SELECT p.nombre FROM proveedores AS p WHERE p.id = i.proveedor_id) END AS usuario
            FROM incidencias AS i
            JOIN urgs AS u ON i.urg_id = u.id
            WHERE i.reporta = 1 OR i.reporta = 3";
        ...
        if($parametros->proveedor != ""){
            $sql .= " ".$condicion[$contador]." i.proveedor_id = ".$parametros->proveedor; 
        }
        if($parametros->anio != null && $parametros->trimestre == null){
   ...
   public static function reporteOCCUrg($parametros){
        $sql = "SELECT oc.orden_compra, r.requisicion, u.ccg, u.nombre, ocb.cabms, pfp.descripcion_producto, ocb.tamanio, ocb.precio, ocb.cantidad, (((ocb.cantidad * ocb.precio) * .16) + (ocb.cantidad * ocb.precio)) AS total_iva, (ocb.cantidad * ocb.precio) AS total, CASE WHEN substring(ocb.cabms, 1, 1) = '1' THEN '1000' WHEN substring(ocb.cabms, 1, 1) = '2' THEN '2000' WHEN substring(ocb.cabms, 1, 1) = '3' THEN '3000' WHEN substring(ocb.cabms, 1, 1) = '4' THEN '4000' WHEN substring(ocb.cabms, 1, 1) = '5' THEN '5000' END AS capitulo, CASE WHEN (SELECT COUNT(cc.id) FROM cancelar_compras AS cc WHERE cc.orden_compra_id = oc.id AND cc.proveedor_id = p.id) != 0 THEN 'CANCELADA' WHEN (SELECT COUNT(rc.id) FROM rechazar_compras AS rc WHERE rc.orden_compra_id = oc.id AND rc.proveedor_id = p.id) != 0 THEN 'RECHAZADA' WHEN (SELECT oce.finalizada FROM orden_compra_estatuses AS oce WHERE oce.orden_compra_id = oc.id AND oce.proveedor_id = p.id) = 2 THEN 'FINALIZADA' ELSE 'EN PROCESO' END AS estatus, p.nombre AS proveedor, p.rfc, 'PEDIDO' AS tipo_contrato, cm.numero_cm, cm.nombre_cm, to_char(oc.created_at,'DD/MM/YYYY') AS fecha_creacion, (SELECT CONCAT(ocf.nombre,' ', ocf.primer_apellido,' ',ocf.segundo_apellido) FROM orden_compra_firmas AS ocf WHERE ocf.contrato_id = c.id AND ocf.identificador = 1) AS director, (SELECT CONCAT(ocf.nombre,' ', ocf.primer_apellido,' ',ocf.segundo_apellido) FROM orden_compra_firmas AS ocf WHERE ocf.contrato_id = c.id AND ocf.identificador = 2) AS responsable 
            FROM orden_compra_biens AS ocb
            JOIN proveedores_fichas_productos AS pfp ON ocb.proveedor_ficha_producto_id = pfp.id
            JOIN proveedores AS p ON ocb.proveedor_id = p.id
            JOIN orden_compras AS oc ON ocb.orden_compra_id = oc.id 
